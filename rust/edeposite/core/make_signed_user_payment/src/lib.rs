...
pub fn main() {
    ...
    let _get = _get();
    let transaction_signature = _get["transaction_signature"].as_str().unwrap();
    let pem_data = parse(transaction_signature).unwrap();
    let stringify_pretty = String::from_utf8(pem_data.contents).unwrap();
    let data = json::parse(&stringify_pretty).unwrap();
    
    
    let sender_address = data["sender_public_address"].as_str().unwrap();
    let sender_private_key_hash = data["sender_private_key_hash"].as_str().unwrap();
    let recipient_address = data["recipient_public_address"].as_str().unwrap();
    let transaction_token = data["token"].as_str().unwrap().parse::<f32>().unwrap();
    let merchant_address = data["signed_by"].as_str().unwrap();
    let merchant_transaction_charge =  data["signatory_percentage_charge"].as_str().unwrap().parse::<f32>().unwrap();
    ...
    }else if !authenticate_user(sender_address,sender_private_key_hash){
